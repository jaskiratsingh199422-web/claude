...
def answer_question(message, history):
    api_key = get_api_key()
    if not api_key:
        return ("⚠️ The AI backend isn't configured yet. An admin needs to add "
    ...
    try:
        ...
        return f"⚠️ Something went wrong reaching the AI service: {e}"
...
if prompt:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Checking HR policies..."):
            reply = answer_question(prompt, st.session_state.messages[:-1])
        st.markdown(reply)
